	...
	<%
		...
		try {
			List<ProjectVO> projectList = adminBSO.getProjectList(
					Long.parseLong((String) session.getAttribute("userType")),
					Long.parseLong((String) session.getAttribute("userRegId")));
				...
				<tbody>
					<c:forEach items="${projectList}" var="element">
						<tr>
							...
							<td><a
								href="<%=request.getContextPath()%>/jsp/AddActual.jsp?projectId=${element.projectId}">List</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
