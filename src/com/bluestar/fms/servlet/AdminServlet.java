		...
		try {
			if (addmodule != null && addmodule.trim().length() > 0) {
				...
				if (action != null && action.equals("linkmanager")) {
					...
					List<ExchageRateVO> exchangeRateVOList = adminBSO
							.getExchangeRateVOList(exmodule);
		...
		try {
			...
			if ("add".equalsIgnoreCase(action)) {
				...
				if ("manager".equalsIgnoreCase(module)) {
					...
					if (daoManager.create(manager) != null) {
				...
				if ("account".equalsIgnoreCase(module)) {
					...
					if (daoManager.create(account) != null) {
