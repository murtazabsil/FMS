...
public class AdminUtil {
	...
	public static Project convertProjectVOtoEntity(ProjectVO projectvo) {
		...
		if (projectvo != null) {
			...
			account.setAccountId(converBigItegerToLong(projectvo
					.getProjectAccount()));
			project.setProjectAccount(account);
			Currency currency = new Currency();
			currency.setCurrencyId(converBigItegerToLong(projectvo
					.getProjectBaseCurrency()));
			project.setProjectBaseCurrency(currency);

			Priority priority = new Priority();
			priority.setPriorityId(converBigItegerToLong(projectvo
					.getProjectPriority()));
			project.setProjectPriority(priority);

			Status status = new Status();
			status.setStatusId(converBigItegerToLong(projectvo
					.getProjectStatus()));
			...
			// BigInteger(projectvo.getParameter("priority")));
	...
	public static Manager convertManagerVOtoEntity(ManagerVO managervo) {
		...
		if (managervo != null) {
			...
			Account account = new Account();
			account.setAccountId(converBigItegerToLong(managervo
					.getManagerAccountId()));

			Currency currency = new Currency();
			currency.setCurrencyId(converBigItegerToLong(managervo
					.getManagerCurrency()));
			...
			Location location = new Location();
			location.setLocationId(converBigItegerToLong(managervo
					.getManagerLocationId()));
			manager.setManagerLocationId(location);
			// manager.setManagerPriority(new
			// BigInteger(managervo.getParameter("priority")));
	...
	public static User convertUserVOtoEntity(UserVO userVO) {
		...
		if (userVO != null) {
			...
			usertType.setUserTypeId(converBigItegerToLong(new BigInteger(userVO
					.getUserType() + "")));
	...
	public static ProjectVO convertProjectEntityToVO(Project project) {
		...
		if (project != null) {
			...
			projectVO.setProjectLob(converLongToBigInteger(project
					.getProjectLob().getLobId()));
			projectVO.setProjectLobName(project.getProjectLob().getLobName());

			projectVO.setProjectAccount(converLongToBigInteger(project
					.getProjectAccount().getAccountId()));
			projectVO.setProjectAccountName(project.getProjectAccount()
					.getAccountName());

			projectVO.setProjectBaseCurrency(converLongToBigInteger(project
					.getProjectBaseCurrency().getCurrencyId()));
			projectVO.setProjectBCName(project.getProjectBaseCurrency()
					.getCurrencyName()
					+ ","
					+ project.getProjectBaseCurrency().getCurrencyCode());

			projectVO.setProjectPriority(converLongToBigInteger(project
					.getProjectPriority().getPriorityId()));
			projectVO.setProjectPriorityName(project.getProjectPriority()
					.getPriorityCode());

			projectVO.setProjectStatus(converLongToBigInteger(project
					.getProjectStatus().getStatusId()));
			projectVO.setProjectStatusName(project.getProjectStatus()
					.getStatusCode());

			projectVO.setProjectType(converLongToBigInteger(project
					.getProjectType().getTypeId()));
			projectVO
					.setProjectTypeName(project.getProjectType().getTypeCode());
			...
			// projectVO.setProjectPriority(new
			// BigInteger(project.getParameter("priority")));
	...
	public static ManagerVO convertManagerEntityToVO(Manager manager) {
		...
		if (manager != null) {
			...
			managerVO.setManagerLobId(converLongToBigInteger(manager
					.getManagerLobId().getLobId()));
			managerVO.setManagerLobName(manager.getManagerLobId().getLobName());

			managerVO.setManagerAccountId(converLongToBigInteger(manager
					.getManagerAccountId().getAccountId()));
			managerVO.setManagerAccountName(manager.getManagerAccountId()
					.getAccountName());

			managerVO.setManagerCurrency(converLongToBigInteger(manager
					.getManagerCurrency().getCurrencyId()));
			managerVO.setManagerCurrenceName(manager.getManagerCurrency()
					.getCurrencyName()
					+ ","
					+ manager.getManagerCurrency().getCurrencyCode());
			...
			managerVO.setManagerLocationId(converLongToBigInteger(manager
					.getManagerLocationId().getLocationId()));
			managerVO.setManagerLocationName(manager.getManagerLocationId()
					.getLocationName());
			// managerVO.setManagerPriority(new
			// BigInteger(manager.getParameter("priority")));
	...
	public static List<LocationVO> getLocationVOListFromEntityList(
		...
		for (Location location : listLocation) {
			System.out
					.println("in getLocationVOListFromEntityList location not null");
