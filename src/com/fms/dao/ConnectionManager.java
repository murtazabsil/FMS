...
public class ConnectionManager {
	...
	public static Session getSession(String connStr) {
		...
		try {
			if (sessionFactoryMap.size() > 0) {
				...
				session = sessionFactoryMap.get(connStr).openSession();
			} else {
				debugLogger("Creating New Session");
				SessionFactory sessionFactory = new AnnotationConfiguration()
						.configure(connStr).buildSessionFactory();
		...
		try {
			if (connStr != null || !connStr.equals("")) {
				...
				if (isConnAvailable == false) {
					...
					debugLogger("SessionFactory for given " + connStr
							+ " exist and returning it.");
	...
	private static synchronized SessionFactory loadFactory(String connStr) {
		...
		try {
			if (sessionFactoryMap != null && !sessionFactoryMap.isEmpty()
					&& sessionFactoryMap.containsKey(connStr)) {
				sessionFactory = sessionFactoryMap.get(connStr);
				return sessionFactory;
			} else {
				...
				sessionFactory = new AnnotationConfiguration().configure(
						connStr).buildSessionFactory();
				if (sessionFactory != null) {
					debugLogger("SessionFactory created successfully for connStr="
							+ connStr);
					sessionFactoryMap.put(connStr, sessionFactory);
				} else {
					debugLogger("Unable to create sessionFactory for given connStr="
							+ connStr);
				}
			}
	...
	/*
	 * public static Session getSession(String connStr) {
	 * debugLogger("getSession start for given connStr=" + connStr); Session
	 * session = null; try { SessionFactory sessionFactory =
	 * getSessionFactory(connStr); if (sessionFactory != null) { if (session ==
	 * null) { session = sessionFactory.openSession();
	 * System.out.println("In getSession Created New Session :" + connStr); }
	 * else if(!session.isOpen()) { session = sessionFactory.openSession();
	 * System.out.println("In getSession Existing Session :" + connStr); } else
	 * { System.out.println("In getSession Thread Local Session :" + connStr); }
	 * } } catch (Exception e) { e.printStackTrace();// remove this when logger
	 * implemented } return session; }
	 */
		...
		try {
			...
			if (sessionFactory != null) {
				if (session == null) {
					...
					System.out.println("In getNewSession Created New Session :"
							+ connStr);
				} else {
					session = sessionFactory.openSession();
					System.out.println("In getNewSession Created New Session :"
	...
	private static void validateConnection(Session session, String connStr) {
		...
		try {
			...
			System.out
					.println("Exception in validateConnection ****************:"
							+ e);
